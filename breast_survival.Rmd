...
curatedBreastData contains 34 high-quality GEO gene expression microarray datasets,
...
### - `eset@featureData@data` or fData()
...
selected=which(dims[,2]==max(dims[,2]))
names(list.eset)[selected]
...
* __Censoring__ 
no observation (e.g. end of following/no event/)

* __Events in cancer studies__
    * Relapse
    * Death
...
url="http://dni-institute.in/blogs/wp-content/uploads/2015/02/cox_regression_output.png"
```
...
```{r age, include=F}
  ...
  fit <- survfit(Surv(clinical$clinical.OS_months_or_MIN_months_of_OS,clinical$clinical.OS) ~ (clinical$clinical.age>50),
                 data = data.frame(clinical))
